 ...
 *
 * Colour comes from `currentColor`, so `className="text-emerald-600"` styles it. Passing
 * `shimmer` paints it with the site's sweeping foreground/primary gradient instead, which
 * is the SVG equivalent of the `.animate-shimmer` utility in globals.css — a background
 * clipped to text cannot reach glyphs that are drawn as paths.
 */

const OPENTYPE_CDN = "https://cdn.jsdelivr.net/npm/opentype.js@1.3.4/dist/opentype.min.js";

// Self-hosted copy of the 21st.dev mirror asset the component shipped with. The font has
// to be CORS-readable to be parsed, and serving it from our own origin means the heading
...
export interface HandwritingTextProps {
  ...
  height?: string;
  /** Paint with the site's sweeping foreground/primary gradient. */
  shimmer?: boolean;
  className?: string;
}
...
export function HandwritingText({
  ...
  const gradientId = `hw-shimmer-${useId().replace(/[^a-zA-Z0-9]/g, "")}`;
  const paint = shimmer ? `url(#${gradientId})` : "currentColor";
  ...
  useEffect(() => {
    ...
    // Vertically the box comes from the font's own ascender/descender, not from the ink
    // of this particular word. Measuring the ink would make a word without a descender
    // render larger than one with, so two of these side by side would disagree on letter
    // size and sit off each other's baseline.
    const scale = EM / font.unitsPerEm;
    const top = EM - font.ascender * scale - pad;
    const bottom = EM - font.descender * scale + pad;
  ...
  return (
    <svg
      ...
      style={{
        ...
        // The width is a fixed length, so on a narrow screen a long phrase would push
        // the page sideways. The viewBox scales the glyphs down to fit instead.
        maxWidth: "100%",
        overflow: "visible",
      }}
    >
      {shimmer && (
        <defs>
          {/* Half the box wide and repeated, so translating it by one box width sweeps
              the highlight across exactly as the CSS utility's background-position does. */}
          <linearGradient id={gradientId} x1="0" y1="0" x2="0.5" y2="0" spreadMethod="repeat">
            <stop offset="0" style={{ stopColor: "hsl(var(--foreground))" }} />
            <stop offset="0.5" style={{ stopColor: "hsl(var(--primary))" }} />
            <stop offset="1" style={{ stopColor: "hsl(var(--foreground))" }} />
            <animateTransform
